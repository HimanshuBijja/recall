...
analytics/import upgrades**, the **Web text capture (any site)** feature, the **Mobile UI and Layout fixes**, the **Extension Multi-line Flashcard & Gemini Prompt Integrity** improvements, the **Selective card testing filter + unified color kind badges**, the **Extension Chatbot Timeout & Typecheck Fixes**, the **Test Session Explanation Markdown Rendering Fix**, the **Extension Capture Timeout Increase**, the **Subscript/Superscript Prompt Restrictions**, the **Match Card Pairs Markdown Rendering Fix**, the **Markdown Option Rendering Fix for all Card Types**, the **Subject/Group-specific FSRS due queues + empty group soft-delete protection and Subject detail layout styling + group search bar upgrades**, the **Video Frame Capture Compression** upgrade, the **Unified wide card edit layout & auto-resizing textareas** upgrade, the **Submit test session early** upgrade, the **Windows Startup Daemon & Boot Timing** fix (`setup-shortcuts.ps1` user APPDATA targeting, `mirror-pull.mjs` offline try-catch resilience, `open-app.vbs` active polling), the **YouTube Deep-Link Timestamp Button** (`▶ YouTube mm:ss`) feature, the **Standalone Notes Revision Engine** (`/notes`, Notebook Index chapter re-ordering, full-width slide reader, Quick Index Drawer [Feature A], Notebook Export to Markdown [Feature E], LocalStorage card kind filter persistence), the **Pure Screenshot PDF Export Engine** (`window.print()`, Page 1 TOC, Chrome/Edge sidebar chapter outlines, 1/2/4 slides per page layout, and invisible `Ctrl+F` search layer), and the **Master Search Command Palette** (`Ctrl+K` shortcut, deep full-text card search, keyboard navigation, scope toggle, Topic Study Filtered Deck mode, and keyword text highlighting).
